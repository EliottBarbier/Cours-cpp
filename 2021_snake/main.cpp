...
int main(int, char**) {
    ...
    std::pair<int,int> initial = snake_init(snake,snake_size);
    int x = initial.first;
                 ...
                 //ligne droite quand on ne touche à rien/
    {
        ...
        // Lecture clavier et actions
         //Il faut que ça soit le même que celui du keyboard
        ...
        if (snake_eat_himself(snake)){ //S'il se mange soit même
            ...
            char mon_char;
            std::cin >> mon_char;
            if (mon_char=='n' or mon_char =='N'){
                bool_for_while=false;
            }
            else if(mon_char=='y' or mon_char=='Y'){
                //Changer le snake pour le remettre emplacement initial et générer un fruit : Y REFLECHIR
                posi_of_fruit=posi_fruit(coordhaut,coordbas);
                std::pair<int,int> initial = snake_init(snake,snake_size);
                x = initial.first;
                y = initial.second;
